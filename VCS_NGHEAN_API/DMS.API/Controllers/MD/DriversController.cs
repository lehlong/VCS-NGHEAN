...
namespace DMS.API.Controllers.MD
{
    ...
    {
        public readonly IDriversService _service;
        ...
        [HttpGet("GetAll")]
        public async Task<IActionResult> GetAll([FromQuery] DriversFilterLite filter)
        {
            var transferObject = new TransferObject();
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] tblDriversDto Drivers)
        {
            var transferObject = new TransferObject();
            Drivers.Id = Guid.NewGuid();
            var result = await _service.Add(Drivers);
            if (_service.Status)
            {
                transferObject.Data = result;
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] tblDriversDto Drivers)
        {
            var transferObject = new TransferObject();
            await _service.Update(Drivers);
            if (_service.Status)
            {
                transferObject.Status = true;
                transferObject.MessageObject.MessageType = MessageType.Success;
        ...
        [HttpGet("GetDetail")]
        public async Task<IActionResult> GetDetail([FromQuery] Guid code)
        {
            var transferObject = new TransferObject();
