...
namespace DMS.BUSINESS.Services.MD
{
    ...
    {
        Task<IList<tblMixerDto>> GetAll(MixerFilterLite filter);
    }
    public class MixerService : GenericService<tblMdMixer, tblMixerDto>, IMixerService
    {
        ...
        public async Task<IList<tblMixerDto>> GetAll(MixerFilterLite filter)
        {
            try
            {
                var query = this._dbContext.tblMdMixer.AsQueryable();
                ...
                query = query.OrderBy(x => x.Code);
                return _mapper.Map<IList<tblMixerDto>>(await query.ToListAsync());
            }
            catch (Exception ex)
            {
                this.Status = false;
